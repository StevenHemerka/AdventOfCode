...
ForEach ( $Instruction in $FormattedMovementInstructions ) {
    ...
    #$CratesToMoveNotReversed = -join ($CratesToMove[-1.. - ($CratesToMove.Length)])
    $CratesToMoveNotReversed = $CratesToMove

    $FormattedSupplyStacks[$StackDestination] = $CratesToMoveNotReversed + $FormattedSupplyStacks[$StackDestination]
