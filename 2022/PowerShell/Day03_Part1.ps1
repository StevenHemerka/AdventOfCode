...
$Day03InputSplit = $Day03Input -split "`n"

[Int]$TotalItemMatchingCharPriority = 0
ForEach ($Items in $Day03InputSplit) {

    $Items = $Items.Trim()
    $ItemsLengthHalf = $Items.Length / 2
    $Item1CharArray = ($Items.Substring(0, $ItemsLengthHalf)).ToCharArray()
    $Item2CharArray = ($Items.Substring($ItemsLengthHalf, $ItemsLengthHalf)).ToCharArray()
    $ItemMatchingChar = Compare-Object -ReferenceObject $Item1CharArray -DifferenceObject $Item2CharArray -IncludeEqual -ExcludeDifferent | Select-Object -ExpandProperty InputObject -Unique

    If ($ItemMatchingChar -cmatch "[a-z]") { 

        $ItemMatchingCharPriority = [byte][char]$ItemMatchingChar - 96

    } Else {

        $ItemMatchingCharPriority = [byte][char]$ItemMatchingChar - 38
...
"Question: What is the sum of the priorities of those item types? Answer: $TotalItemMatchingCharPriority."